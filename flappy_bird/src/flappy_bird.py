...
class Score(GameScreen):
    ...
    def get_score(self, pipe, bird):

        check_pipe = pipe.pipes[0]
        top_pipe = check_pipe['top_pipe_rect'].union(check_pipe['top_pipe_cup_rect'])

        if not self.ignore_pipe and top_pipe.right < bird.bird_rect.left:
            self.game_score += 1
    ...
    def display(self, screen, pipe, bird):
        self.get_score(pipe, bird)
